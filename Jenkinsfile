...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="dlwlsah27/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/jmlee85/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/jmlee85/guestbook-config.git'
                }
            }
        }
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_dlwlsah27') {
                        oDockImage.push()
                    }
                }
            }
